...
public class StatementMover extends FixTemplate {
    ...
    public void generatePatches() {
        ...
        if (Checker.isReturnStatement(stmtType) || Checker.isBreakStatement(stmtType) || Checker.isContinueStatement(stmtType) || Checker.isVariableDeclarationStatement(stmtType)) {
            return;
        }
        ...
        if (Checker.isSwitchStatement(parentNodeType)) {
            ...
            if (Checker.isCatchClause(suspCodeTree.getType())
                    || (Checker.isBlock(suspCodeTree.getType()) && "FinallyBody".equals(suspCodeTree.getLabel()))) {
                return;
            }
