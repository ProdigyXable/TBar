        ...
        for (ITree child : children) {
            ...
            if (Checker.isComplexExpression(childType)) {
                ...
                if (Checker.isInfixExpression(childType)) {
                    ...
                    if (operatorType == 4) {
                        if (conditionalOperators.contains(op)) {
                            conditionalOpExps.add(new Pair<ITree, String>(child, op));
                        }
                    } else {
                        ...
                        if ("&".equals(op) || "|".equals(op)) {
                            bitIorExps.add(new Pair<ITree, String>(child, op));
                        }
                        if ("&=".equals(op) || "|=".equals(op)) {
                            bitAssignExps.add(new Pair<ITree, String>(child, op));
                        }
                    }
...
//					if ("!".equals(op) && condOp != null) conditionalOpExps.add(new Pair<ITree, String>(child, op));
    ...
    private class DLSDeadLocalStoreInReturn {
        ...
        protected void generatePatches(ITree suspCodeTree) {
            if (!Checker.isReturnStatement(suspCodeEndPos)) {
                return;
            }
    ...
    private boolean arePotentialBuggyOperators(String op1, String op2) {
        ...
        if (("+".equals(op1) || "-".equals(op1) || "*".equals(op1) || "/".equals(op1) || "%".equals(op1) || ">>".equals(op1) || "<<".equals(op1))
                && ("+".equals(op2) || "-".equals(op2) || "*".equals(op2) || "/".equals(op2) || "%".equals(op2) || ">>".equals(op2) || "<<".equals(op2))) {
            return true;
        }
        return false;
    }
    ...
    private class ConditionalExpressionOperatorMutator {
                ...
                for (Pair<ITree, String> relationalOperatorPair : relationalOpExps) {
                    ...
                    if (codePart2.startsWith("null") || codePart1.endsWith("null")) {
                        ...
                        for (String relationalOperator : relationalOperators) {
                            if (relationalOperator.equals(op)) {
                                continue;
                            }
        ...
        private void furtherMutation(ITree suspExpTree, String buggyOp) {
            ...
            while (true) {
                ...
                if (Checker.isInfixExpression(parentTreeType)) {
                    break;
                }
                if (!Checker.isParenthesizedExpression(parentTreeType)) {
                    return;
                }
            ...
            // (a op b) ||/&& (c op d)
            // a op b ||/&& c op d
            if (triedInfixExps.contains(parentTree)) {
                return;
            }
            ...
            if (index == 1) {
                ...
                if (anotherSuspExpTree.getChildren().isEmpty()) {
                    return;
                }
            ...
            if (Checker.isInfixExpression(anotherSuspExpTree.getType()) && buggyOp.equals(anotherSuspExpTree.getChild(1).getLabel())) {
                ...
                for (String relationalOperator : relationalOperators) {
                    if (relationalOperator.equals(buggyOp)) {
                        continue;
                    }
    ...
    /**
     * Fix pattern for BC_VACUOUS_INSTANCEOF violations. - if ( exp instanceof
     * String ) { + if ( exp != null) {
     *
     * @author kui.liu
     */
    ...
    private class InstanceofMutator {
        ...
        protected void generatePatches(ITree suspCodeTree) {
            if (Checker.isIfStatement(suspCodeTree.getType())) {
                ...
                if (!Checker.isInstanceofExpression(exp.getType())) {
                    return;
                }
    ...
    private class AssignmentOperatorMuataor {
        ...
        protected void generatePatches(ITree suspCodeTree) {
            ...
            if ("=".equals(op.getLabel())) {
                ...
                if (assignmentExp.getChildren().size() < 3) {
                    return;
                }
                ITree exp = assignmentExp.getChild(2);
                int pos1 = var.getPos() + var.getLength();
    ...
    /**
     * Fix pattern for NS_DANGEROUS_NON_SHORT_CIRCUIT, BIT_IOR, BIT_SIGNED_CHECK
     * violations.
     *
     * @author kui.liu
     *
        ...
        protected void generatePatches() {
            ...
            for (Pair<ITree, String> bitIorExpPair : bitIorExps) {
                ...
                if ("|".equals(bitIorExpPair.getSecond())) {
                    ...
                    if (!Checker.isParenthesizedExpression(parentTree.getType())) {
                        continue;
                    }
                    parentTree = parentTree.getParent();
    ...
    /**
     * Fix pattern for IM_AVERAGE_COMPUTATION_COULD_OVERFLOW,
     * IM_BAD_CHECK_FOR_ODD violations.
     *
     * @author kui.liu
     *
     */
