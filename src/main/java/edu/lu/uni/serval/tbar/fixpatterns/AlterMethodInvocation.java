...
/**
 * The method invocations are limited to all methods defined in the current
 * program.
 *
 * @author kui.liu
 *
...
//					}
     ...
     *
     * @param parameters
     * @return Empty_ArrayList - zero parameter. other - parameter types.
     */
...
//				}
            ...
            if (dataType == null) {
                dataType = this.varTypesMap.get("this." + paraLabel);
            }
            if (dataType == null) {
                // TODO
                return "Object";
...
//			return "Object"; // TODO
    ...
    private String readArrayAccessParameterReturnType(ITree para) {
        ...
        int i = 1;
        while (true) {
            if (!Checker.isArrayAccess(exp.getType())) {
                break;
            }
            exp = exp.getChild(0);
            i++;
    ...
    protected List<MethodInvocationExpression> identifySuspiciousMethodInvocations2() {
        ...
        for (Map.Entry<ITree, Integer> entry : this.suspMethodInvocations.entrySet()) {
            ...
            if (Checker.isConstructorInvocation(type)) {
                ...
                if (Checker.isAnonymousClassDeclaration(parameters.get(size - 1).getType())) {
                    continue;
                }
                int i = 0;
...
//					rootTree = this.classDeclarationAst;
                    ...
                    if (indexPos == 0) { // the method belongs to the current class or its ancestral classes.
                        ...
                        if (Checker.isSimpleName(prePeerCodeAstType)) { // a variable.
                            ...
                            if (varType == null) {
                                varType = this.varTypesMap.get("this." + varName);
                            }
                            if (varType == null) {
                                varType = varName;
                            }
...
//							rootTree = this.classDeclarationAst;
                            ...
                            while (true) {
                                if (!Checker.isArrayAccess(arrayVarTree.getType())) {
                                    break;
                                }
                            ...
                            if (Checker.isSimpleName(arrayVarType)) {
                                ...
                                if (varType == null) {
                                    continue; // It seems impossible as well.
                                }//								rootTree = this.classDeclarationAst;
            ...
            if (rootTree == null || varType == null) {
                continue;
            }

            // Read parameter data types.
            List<String> paraTypeStrs = readMethodParameterTypes(parameters);
            if (paraTypeStrs == null) {
                continue; // Generate ERROR when reading its parameter types.
            }
            ...
            if (Checker.isFieldDeclaration(child.getType())) { // Field declaration
                ...
                for (int i = 1, size = subChildren.size(); i < size; i++) {
                    ...
                    if (Checker.isVariableDeclarationFragment(varDeclaration.getType())) {
                        if (varType == null) {
                            varType = subChildren.get(i - 1).getLabel();
                        }
            ...
            for (ITree child : children) {
                if (Checker.isMethodDeclaration(child.getType())) {
                    ...
                    if (paraStr.startsWith("null")) {
                        ...
                        if (indexExp > 0) {
                            paraStr = paraStr.substring(0, indexExp);
                        }
                    }
                        ...
                        if (paraStr == null) {
                            if (currentMethodName.equals(methodName)) {
                                ...
                                if (methodNamesList == null) {
                                    methodNamesList = new ArrayList<String>();
                                }
