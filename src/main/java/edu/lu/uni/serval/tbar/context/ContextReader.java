...
public class ContextReader {
    ...
    /**
     * Read the all variables by traversing the ancestral tree of the suspicious
     * code AST.
     *
     * @param codeAst
    ...
    private static void addFieldsToVars(List<Field> fields, Map<String, List<String>> allVarNamesMap,
        ...
        for (Field field : fields) {
            ...
            if (!privateNeeded && mt == ModifierType.PRIVATE) {
                continue;
            }
            ...
            if (!varTypesMap.containsKey(varName)) {
                varTypesMap.put(varName, dataType);
            }
    ...
    private static void addStaticFieldsFromDependencies(Map<String, List<String>> allVarNamesMap,
        ...
        for (String dependency : dependencies) {
            if (dependency.equals(dic.findSuperClassName(classNameAndPath))) {
                continue;
            }
...
//				}
    ...
    public static String readClassNameAndPath(ITree classDeclarationTree) {
        ...
        while (true) {
            if (Checker.isCompilationUnit(parentTree.getType())) {
                break;
            }
        ...
        if (!Checker.isPackageDeclaration(packageTree.getType())) {
            return className;
        }
        ...
        if (index >= 0) {
            packageName = packageName.substring(index + 8);
        }
...
//							break;
    ...
    /**
     * Remove the type parameters of the data type. e.g., List<T> --> List.
     *
     * @param returnType
     * @return
     */
    ...
    /**
     * Remove the type parameters of the data type (class name). e.g., List<T>
     * --> List.
     *
     * @param className
     * @return
    ...
    public static void identifySuspiciousVariables(ITree suspCodeAst, List<ITree> varTrees, List<String> allSuspVariables) {
        ...
        for (ITree child : children) {
            ...
            if (Checker.isSimpleName(childType)) {
                ...
                if (!allSuspVariables.contains(qualifiedName)) {
                    allSuspVariables.add(qualifiedName);
                }
                ...
                if (!allSuspVariables.contains(nameStr)) {
                    allSuspVariables.add(nameStr);
                }
    ...
    public static boolean containsVar(ITree codeAst, List<String> varNames) {
        if (varNames.contains(codeAst.getLabel())) {
            return true;
        }
        List<ITree> children = codeAst.getChildren();
        ...
        for (ITree child : children) {
            ...
            if (Checker.isSimpleName(childType)) {
                if (child.getLabel().startsWith("MethodName:")) {
                    ...
                    if (var != null && varNames.contains(var)) {
                        return true;
                    }
                }
            } else if (Checker.isComplexExpression(childType)) {
