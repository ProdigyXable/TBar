...
public class VariableReplacer extends FixTemplate {
    ...
    public void generatePatches() {
        ...
        if (Checker.isForStatement(stmtType) || Checker.isEnhancedForStatement(stmtType)
                || Checker.isWhileStatement(stmtType) || Checker.isDoStatement(stmtType)) {
            return;
        }
        ...
        for (ITree suspVar : suspVars) {
            if (isForStatementVar(suspVar.getLabel())) {
                continue;//
            }
            String codePart1;
            ...
            if ("boolean".equals(suspVarType)) {
                ...
                if (booleanVars == null) {
                    if (booleanVars_ != null) {
                        booleanVars = allVarNamesMap.get("Boolean");
                    } else {
                        continue;
                    }
                ...
                for (String var : booleanVars) {
                    if (var.equals(suspVarName) || var.equals("this." + suspVarName)) {
                        continue;
                    }
                ...
                if (sameTypeVars != null) {
                    ...
                    for (Pair<String, Double> var : similarVarPairs) {
                        for (ITree suspV : suspVars) {
                            if (suspV.getLabel().equals(var.getFirst())) {
                                continue MyFor;
                            }
                        }
...
//				}
            ...
            if (Checker.isQualifiedName(suspVar.getType())) {
                ...
                for (Map.Entry<String, String> entity : publicStaticFields.entrySet()) {
                    ...
                    if (varType.equals(suspVarType)) {
                        this.generatePatch(codePart1 + publicStaticField + codePart2);
                    }
                }

                ITree sameValueSuspVarTree = sameValueSuspVarTrees.get(suspVarName);
                if (sameValueSuspVarTree == null) {
                    sameValueSuspVarTrees.put(suspVarName, suspVar);
                } else {
                    ...
                    for (Map.Entry<String, String> entity : publicStaticFields.entrySet()) {
                        String publicStaticField = entity.getKey();
                        String varType = entity.getValue();
                        if (publicStaticField.equals(suspVarName) || !varType.equals(suspVarType)) {
                            continue;
                        }
        ...
        for (ITree varTree : suspVars) {
            if (!Checker.isSimpleName(varTree.getType())) {
                continue;
            }
            ...
            if (varDataType == null) {
                varDataType = varTypesMap.get("this." + varName);
            }
            if (varDataType == null) {
                continue;
            }
...
//				}
    ...
    private boolean isForStatementVar(String suspVarName) {
        ...
        while (true) {
            ...
            if (!Checker.isStatement(parentStmt.getType())) {
                return false;
            }
    ...
    private List<Pair<String, Double>> sortCompatibleVars(List<String> sameTypeVars, String suspVarName) {
        ...
        if (index >= 0) {
            suspVarName = suspVarName.substring(index + 1);
        }
