    ...
    public class ExpressionAdder {
        ...
        public void generatePatches(Map<ITree, Integer> allPredicateExpressions) {
            ...
            for (Map.Entry<ITree, Integer> entry : allPredicateExpressions.entrySet()) {
                ...
                if (suspPredicateExpStr.contains(predicateExpCandidateStr) || predicateExpCandidateStr.equals(suspPredicateExpStr)) {
                    continue;
                }
                if (predicateExpCandidateStr.contains("==null") || predicateExpCandidateStr.contains("== null")
                        || predicateExpCandidateStr.contains("!=null") || predicateExpCandidateStr.contains("!= null")) {
                    continue;
                }
                if (triedExpCands.contains(predicateExpCandidateStr)) {
                    continue;
                }
            ...
            if (!Checker.isIfStatement(suspStmtAst.getType())) {
                return;
            }
        ...
        private List<String> readVariables(ITree predicateExp) {
            ...
            for (ITree child : children) {
                if (Checker.isSimpleName(child.getType())) {
                    ...
                    if (var == null) {
                        vars.addAll(readVariables(child));
                    } else {
                        vars.add(var);
                    }
                } else {
            ...
            for (Map.Entry<ITree, Integer> entry : allSuspPredicateExps.entrySet()) {
                ...
                for (Map.Entry<ITree, Integer> entry2 : allPredicateExpressions.entrySet()) {
                    ...
                    if (expCandidateStr.contains("==null") || expCandidateStr.contains("== null")
                            || expCandidateStr.contains("!=null") || expCandidateStr.contains("!= null")) {
                        continue;
                    }
                    if (suspPredicateExpStr.contains(expCandidateStr) || expCandidateStr.equals(buggyExpStr)) {
                        continue;
                    }
                    ...
                    } else if (triedExpCands.contains(expCandidateStr)) {
                        continue;
                    }
